...
module.exports = class MysqlConnector {
    constructor(host, username, password, db, tableName = '') {
        ...
        this.connection.connect(
            (err) => {
                if (err) console.log("\t" + this.chalk.redBright("[ERROR]: " + err));
                else {
    ...
    async query(sql) {
        ...
        return new Promise((resolve, reject) => {
            ...
            (err) => console.log("\t" + this.chalk.redBright("[ERROR]: " + err))
        );
    }
    ...
    async get(fields = [], conditions = {}) {
        if (!this.tableName || this.tableName == '') throw ("Table not specified");
